...
Sexo* ViewAllSexo(int* count) {
    ...
    if (mysql_query(conn, "SELECT * FROM Sexo;")) {
        fprintf(stderr, "%s\n", mysql_error(conn));
    }
    MYSQL_RES *res = mysql_store_result(conn);
    *count = mysql_num_rows(res);
    Sexo* arr = malloc(sizeof(Sexo) * (*count));
    MYSQL_ROW row;
    int i = 0;
...
Sexo ObtainSexo(int id) {
    ...
    MYSQL* conn = get_connection();
    char query[256];
    sprintf(query, "SELECT * FROM Sexo WHERE ID=%d;", id);
...
void DeleteSexo(int id) {
    MYSQL* conn = get_connection();
    char query[256];
    sprintf(query, "DELETE FROM Sexo WHERE ID=%d;", id);
    mysql_query(conn, query);
    mysql_close(conn);
}
