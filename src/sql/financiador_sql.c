...
Financiador* ViewAllFinanciador(int* count) {
    ...
    if (mysql_query(conn, "SELECT * FROM Financiador;")) {
        fprintf(stderr, "%s\n", mysql_error(conn));
    }
    MYSQL_RES *res = mysql_store_result(conn);
    *count = mysql_num_rows(res);
    Financiador* arr = malloc(sizeof(Financiador) * (*count));
    MYSQL_ROW row;
    int i = 0;
...
Financiador ObtainFinanciador(int id) {
    ...
    MYSQL* conn = get_connection();
    char query[256];
    sprintf(query, "SELECT * FROM Financiador WHERE ID=%d;", id);
...
void DeleteFinanciador(int id) {
    MYSQL* conn = get_connection();
    char query[256];
    sprintf(query, "DELETE FROM Financiador WHERE ID=%d;", id);
    mysql_query(conn, query);
    mysql_close(conn);
}
